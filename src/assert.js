...
export let message = function (errorCode, args) {
...
export let reserveCodeRange = function (first, last, moduleName) {
  assert(first <= last, "Invalid code range");
  let noConflict = every(Assert.reservedCodes, function (range) {
...
export let setLocation = function (location) {
  //assert(location, "Empty location");
  Assert.location = location;
}

export let clearLocation = function () {
  Assert.location = null;
}

export let setCounter = function (count, message) {
  Assert.count = count;
  Assert.countMessage = message ? message : "ERROR count exceeded";
}

export let checkCounter = function () {
